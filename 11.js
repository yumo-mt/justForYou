...
class IndexList extends React.Component {
    constructor(props){
        ...
        this.state = {
            list:[],
            outerScroller:null,
            pullToRefreshBox:null,
            scrollList:null,
            preloader:null,
            pullToRefreshArrow:null,
        }
    }
    ...
    componentDidMount() {
        this.fetchData();
        this.setState({
            outerScroller:document.getElementById('outerScroller'),
            pullToRefreshBox:document.getElementById('pullToRefreshBox'),
            scrollList:document.getElementById('scroll'),
            preloader:document.getElementById('preloader'),
            pullToRefreshArrow:document.getElementById('pullToRefreshArrow')
        })
        ...
        this.pullToRefresh(this.state.outerScroller,this.state.pullToRefreshBox,this.state.scrollList,this.state.preloader,this.state.pullToRefreshArrow)
    }
    fetchData(){
    ...
    loadingFinish() {
        this.state.scrollList.style.overflow = 'auto';
        this.state.preloader.style.display = 'none';
        //将页面还原
        var LFT = setInterval(function () {
            this.state.outerScroller.style.top= parseInt(this.state.outerScroller.style.top)-3+'px';
            if( parseInt(this.state.outerScroller.style.top) <= defaultTopVal){
                clearInterval(LFT);
                if(this.state.outerScroller.offsetTop<defaultTopVal){
                    this.state.outerScroller.style.top = defaultTopVal+'px'
                }
                //进行新的一轮监听
                checkState(this.state.outerScroller.offsetTop)
                this.state.pullToRefreshArrow.style.display = 'block';
                removeClass(pullToRefreshBox,'up')
            }
        },10)
    }
    ...
    render() {
        return (
            <main>
                <div className=" outerScroller" id="outerScroller">
                    <div className="pullToRefreshBox" id="pullToRefreshBox">
                        <div className="preloader" id="preloader"></div>
                        <div className="pullToRefreshArrow" id="pullToRefreshArrow"></div>
                    </div>
                    <ul style={{background:"#eee"}} id="scroll" className="scroll">
                        {this.indexList()}
                    </ul>
                </div>
            </main>
