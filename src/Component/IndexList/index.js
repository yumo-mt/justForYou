...
class IndexList extends React.Component {
  ...
  giveStar(e) {
    ...
    let p1 = new Promise((resolve, reject)=> {
      this.props.giveStar(params)
      var timer = setInterval(()=> {
        if (this.props.starContent.data) {
          clearInterval(timer);
          resolve(this.props.starContent.data)
        }
      }, 100)
    })
    p1.then((data)=> {
...
//刷新完成
        ...
        <li className="" style={Styles.indexList} key={item._id}>
            ...
            <div className="list">
              <div className="" style={{paddingTop: '0.4rem'}}>
                ...
                <div style={{display: 'inline-block', verticalAlign: 'top', height: '2rem'}}>
                  ...
                  <div style={{fontSize: '12px'}}><span className="icon icon-clock"> </span> {dateDiff(item.createAt)}
                  </div>
                </div>
          ...
          <div style={{display: 'block', width: '100%', fontSize: '14px'}}>
            <span className="icon icon-star" style={_this.starStyle(item.star)} onClick={(e)=> {
              _this.giveStar(e)
            }} data-articleId={item._id}> {item.star.length}</span>
  ...
  checkLogin() {
    ...
    if (localStorage.getItem('userToken')) {
      ...
      return <div><p><a onClick={this.goLogin} className="button button-big button-fill button-success">登录 </a></p>
      </div>
    }
