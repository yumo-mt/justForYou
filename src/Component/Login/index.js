...
// import {UserModel} from '../dataModel02'
let Styles = {
  header: {
    lineHeight: "2.2rem"
  },
...
class Login extends React.Component {
  ...
  componentWillReceiveProps(nextProps){
    let data = nextProps.registerLogin.data
    if(data.type=='2'&&data.id=='1'){
      $.toast("注册成功");
      localStorage.setItem('userToken',nextProps.registerLogin.data.content)
      location.hash = "/indexList";
    }else if(data.type=='2'&& (data.id == '2'||'3')){
      $.toast(data.content);
    }

    if(data.type == '1'&&data.id=='1'){
      $.toast('登录成功')
      localStorage.setItem('userToken',data.content);
      this.context.router.push('/indexList');
    }else if(data.type == '1'&&data.id=='0'){
      $.toast(data.content)
    }
  }
              ...
              <li>
                <div className="item-content">
                  ...
                  <div className="item-inner">
                    ...
                    <div className="item-input">
                      <input type="password" ref="rpassword" name="rpassword" placeholder="Password"
          ...
          <div className="content-block">
            <div className="row">
              <div className="col-50"><a onClick={()=> {
                this.formReset()
              }} className="button button-big button-fill button-danger">重置</a></div>
              <div className="col-50"><a onClick={()=> {
                this.userRegister()
              }} className="button button-big button-fill button-success" type="submit">注册</a></div>
            </div>
          </div>
        </form>
...
function mapStateToProps(state) {
  return {registerLogin:state.registerLogin}
}
...
export const LoginBox = connect(mapStateToProps,mapDispatchToProps)(Login)
